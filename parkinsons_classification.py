...
def get_device():
    return torch.device("cpu")  # For reproducible results
...
if torch.backends.mps.is_available():
    # MPS doesn't have full determinism control, but this helps
...
if __name__ == "__main__":
    ...
    print("-"*70)
    voice_filepath = '/Users/aakritijain/Desktop/parkinsons_research/parkinsons 2/parkinsons.data'
    voice_data = load_and_preprocess_parkinsons_data(voice_filepath)
    
    # 2. Load Gait Data
    print("\n[2/3] LOADING GAIT DATA...")
    print("-"*70)
    gait_dir = '/Users/aakritijain/Desktop/parkinsons_research/gait-in-parkinsons-disease-1.0.0'
...
pos_weight = torch.tensor([n_controls / n_patients]).to(device)
...
def train_multimodal_model(model, train_loader_voice, train_loader_gait, val_loader_voice, val_loader_gait, num_epochs=50, learning_rate=1e-3, weight_decay=1e-4, device=device, patience=10, verbose=True):
    """
...
test_metrics = evaluate(trained_model, test_loader_voice, test_loader_gait, nn.BCEWithLogitsLoss(), device=device)
# 
...
voice_only_model = VoiceOnlyClassifier(voice_encoder_baseline).to(device)
criterion_voice = nn.BCEWithLogitsLoss(pos_weight=pos_weight.to(device))
...
for epoch in range(FAST_EPOCHS):
    ...
    for voice_features, labels in train_loader_voice:
        voice_features, labels = voice_features.to(device), labels.to(device).unsqueeze(1)
    ...
    with torch.no_grad():
        for voice_features, labels in val_loader_voice:
            val_loss += criterion_voice(voice_only_model(voice_features.to(device)), labels.to(device).unsqueeze(1)).item()
...
with torch.no_grad():
    for voice_features, labels in test_loader_voice:
        logits = voice_only_model(voice_features.to(device))
        probs = torch.sigmoid(logits)
...
gait_only_model = GaitOnlyClassifier(gait_encoder_baseline).to(device)
criterion_gait = nn.BCEWithLogitsLoss(pos_weight=pos_weight.to(device))
...
for epoch in range(FAST_EPOCHS):
    ...
    for i, (gait_windows, labels, _) in enumerate(train_loader_gait):
        ...
        gait_windows, labels = gait_windows.to(device), labels.to(device).unsqueeze(1)
    ...
    with torch.no_grad():
        for gait_windows, labels, _ in val_loader_gait:
            val_loss += criterion_gait(gait_only_model(gait_windows.to(device)), labels.to(device).unsqueeze(1)).item()
...
with torch.no_grad():
    for gait_windows, labels, subject_ids in test_loader_gait:
        logits = gait_only_model(gait_windows.to(device))
        probs = torch.sigmoid(logits)
...
concat_model = SimpleConcatFusion(voice_enc, gait_enc).to(device)
criterion = nn.BCEWithLogitsLoss(pos_weight=pos_weight.to(device))
...
for epoch in range(FAST_EPOCHS):
    ...
    for i, (gait_windows, gait_labels, _) in enumerate(train_loader_gait):
        ...
        min_batch = min(voice_features.size(0), gait_windows.size(0))
        voice_features = voice_features[:min_batch].to(device)
        gait_windows = gait_windows[:min_batch].to(device)
        labels = gait_labels[:min_batch].to(device).unsqueeze(1)
    ...
    with torch.no_grad():
        for gait_windows, gait_labels, _ in val_loader_gait:
            try:
                ...
                voice_features, _ = next(voice_iter)
            min_batch = min(voice_features.size(0), gait_windows.size(0))
            val_loss += criterion(concat_model(voice_features[:min_batch].to(device), gait_windows[:min_batch].to(device)), gait_labels[:min_batch].to(device).unsqueeze(1)).item()
    val_loss /= len(val_loader_gait)
...
with torch.no_grad():
    for gait_windows, gait_labels, _ in test_loader_gait:
        try:
            ...
            voice_features, _ = next(voice_iter)
        min_batch = min(voice_features.size(0), gait_windows.size(0))
        logits = concat_model(voice_features[:min_batch].to(device), gait_windows[:min_batch].to(device))
        probs = torch.sigmoid(logits)
...
# Full Model ROC
test_metrics_full = evaluate(trained_model, test_loader_voice, test_loader_gait, nn.BCEWithLogitsLoss(), device)
